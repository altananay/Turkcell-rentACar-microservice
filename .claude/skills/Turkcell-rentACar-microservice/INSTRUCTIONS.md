...
- **Known-true state at the time of writing** — re-verify each run and update this list when it changes: no `Dockerfile` exists anywhere and no Spring Boot app is containerized; 8 test classes exist and none contains a real assertion; there are zero `@CircuitBreaker` annotations despite resilience4j being on the classpath; all backing-service credentials are committed in plaintext in `docker-compose.yml` (there is no `.env` file — it was removed deliberately, so do not report its absence as a finding); the Spring Boot parent is split 3.0.6 / 3.1.0 across modules; `filter-service` uses the base package `kodlamaio.filterservice` with no `com.` prefix, so a `com.kodlamaio` search silently misses it.
