...
| 15 | Security | No `Authorization` header is propagated on any outgoing OpenFeign call — no `RequestInterceptor` exists anywhere in the repo. Since the gateway does not authenticate and every service validates its own token, `rental-service`'s two `PaymentClient` calls hit `/api/**` → `hasAnyRole("user")` and receive 401; the circuit-breaker fallback converts this to `BusinessException("PAYMENT DOWN")`, so `POST /api/rentals` fails with a message pointing at a healthy payment-service. The `CarClient` calls survive only because `/api/cars/**` is `permitAll` with no HTTP-method restriction. Automatic refund and crash recovery are equally broken, and cannot be fixed by propagating the caller's header because `SagaRecoveryScheduler` runs on a thread with no inbound request | repo-wide grep for `RequestInterceptor`/`Authorization`/`SecurityContextHolder` returns zero hits; `SecurityConfig.java:22-31` vs `PaymentClient.java:12,15`; `SagaRecoveryScheduler.java:22` | Confirmed | High | Medium |
...
**Update — 2026-08-04: Finding #15 fixed.** `rental-service` now authenticates itself to `payment-service` with a Keycloak `client_credentials` service-account token. A new realm role `service` was created and assigned to the service account of a new confidential client `rental-service-client`; `SecurityConfig` gained a matcher placed before the `/api/**` rule restricting `/api/payments/process-rental-payment` and `/api/payments/refund-rental-payment` to `hasRole("service")`, so those two money-moving endpoints are now callable only by the service account and no longer by a logged-in human's token. The token is obtained through an `AuthorizedClientServiceOAuth2AuthorizedClientManager` — deliberately not the servlet-request-bound `DefaultOAuth2AuthorizedClientManager` — so it works identically on a request thread and on the `SagaRecoveryScheduler` thread, which is what makes crash recovery and automatic refund work at all; a header-copying interceptor could never have fixed those paths. The Feign `RequestInterceptor` is scoped to `PaymentClient` alone via `@FeignClient(configuration = …)` on a deliberately unannotated configuration class: a component-scanned one would land in the parent context and silently apply to `CarClient` too, making Keycloak a hard availability dependency of `/api/cars/**` calls that are `permitAll` and work with Keycloak stopped today. See `CLAUDE.md` §11 for the role model and §16 for the traps. The row above is left as-is as the point-in-time finding that motivated the fix.
