...
public class BrandManager implements BrandService {
    ...
    public List<GetAllBrandsResponse> getAll() {
        ...
        var response = brands.stream().map(brand -> mapperService.forResponse().map(brand, GetAllBrandsResponse.class)).toList();
        return response;
    }
    ...
    public GetBrandResponse getById(UUID id) {
        ...
        var response = mapperService.forResponse().map(brand, GetBrandResponse.class);
    ...
    public CreateBrandResponse add(CreateBrandRequest request) {
        var brand = mapperService.forRequest().map(request, Brand.class);
        repository.save(brand);
        var response = mapperService.forResponse().map(brand, CreateBrandResponse.class);
        return response;
    }
    ...
    public UpdateBrandResponse update(UUID id, UpdateBrandRequest request) {
        ...
        brand.setId(id);
        repository.save(brand);
        var response = mapperService.forResponse().map(brand, UpdateBrandResponse.class);
        return response;
    }
